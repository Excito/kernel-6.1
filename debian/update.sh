...
dch -v $new_ver-1 "New upstream version"
dch -r ""
...
git add debian
git commit -m "Released version $new_ver-1"
...
scp ../linux_$new_ver-1_armel.changes excito@repo.excito.org:import
for f in `sed -e '0,/^Files:/d' -e '/^\w/,$d' ../linux_$new_ver-1_armel.changes | awk '{ print $5 }'`; do
    scp -q ../$f excito@repo.excito.org:import
    rm ../$f
done
rm ../linux_$new_ver-1_armel.changes

ssh excito@repo.excito.org /home/excito/bin/reprepro -b /home/excito/repo --ignore=wrongdistribution include bookworm import/linux_$new_ver-1_armel.changes
